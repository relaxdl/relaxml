...
def normalise_charset_name(s):
    ...
    if not isinstance(s, str):
        s = s.decode('ascii')
    return (codecs.lookup(s).name.replace("_", "-").upper().replace(
        "ISO8859", "ISO-8859"))
...
def serialise_FG(value, context):
    ...
    if value is None:
        return b""
    flags, name = value
    return str(flags).encode('ascii') + b":" + serialise_simpletext(
        name, context)
...
def interpret_LB_list(values, context):
    ...
    for s in values:
        ...
        result.append(
            (interpret_point(point,
                             context), interpret_simpletext(label, context)))
    return result
...
def serialise_LB_list(values, context):
    ...
    return [
        b":".join(
            (serialise_point(point,
                             context), serialise_simpletext(text, context)))
