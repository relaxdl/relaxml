...
class Node:
    ...
    def __str__(self):
        ...
        def format_property(ident, values):
            return ident.decode(encoding) + "".join("[%s]" % s.decode(encoding)
                                                    for s in values)
        return "\n".join(
...
class Sgf_game:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.root.set_raw(b'SZ',
                          str(self.size).encode(self.presenter.encoding))
    ...
    def get_player_name(self, colour):
        ...
        try:
            return self.root.get({
                'b': b'PB',
                'w': b'PW'
            }[colour]).decode(self.presenter.encoding)
        except KeyError:
            return None
    ...
    def get_winner(self):
        ...
        """
        try:
            colour = self.root.get(b"RE").decode(
                self.presenter.encoding)[0].lower()
        except LookupError:
            return None
        if colour not in ("b", "w"):
            return None
