...
实现说明:
https://tech.foxrelax.com/nlp/sentiment_analysis_rnn/
"""
...
class BiRNN(nn.Module):
    """
    >>> vocab_size, embed_size, num_hiddens, num_layers = 10000, 100, 100, 2
    ...
    def __init__(self, vocab_size: int, embed_size: int, num_hiddens: int,
                 num_layers: int, **kwargs: Any) -> None:
        """
        参数:
        vocab_size: 词典大小
        embed_size: 预训练的词向量维度
        num_hiddens: LSTM隐藏层的大小
        num_layers: LSTM隐藏层的层数
        """
        super(BiRNN, self).__init__(**kwargs)
        self.embedding = nn.Embedding(vocab_size, embed_size)  # 预训练的GloVe
        ...
        # 这里是: 4 * num_hiddens的原因:
        # 连结`初始时间步`和`最终时间步`2个时间步的隐状态, 作为全连接层的输入,
        # 由于我们使用的是双向LSTM, 每个时间步输出的维度是2*num_hiddens, 所以将
        # 这两个时间步连接起来的维度就是4 * num_hiddens
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        返回:
        outs的形状: [batch_size, 2]
        """
        # 因为LSTM网络要求其输入的第一个维度是时间维(num_steps),
        # 所以在获得token表示之前, 输入会被转置
        # embeddings.shape [num_steps, batch_size, num_hiddens]
        ...
        # 连结`初始时间步`和`最终时间步`2个时间步的隐状态, 作为全连接层的输入
        # outputs[0].shape [batch_size, 2*num_hidden]
        # outputs[1].shape [batch_size, 2*num_hidden]
        # encoding.shape [batch_size, 4*num_hidden]
        encoding = torch.cat((outputs[0], outputs[-1]), dim=1)

        # outs.shape [batch_size, 2]
...
def bi_rnn_net(vocab: Vocab, embed_size: int, num_hiddens: int,
               num_layers: int) -> BiRNN:
...
if __name__ == '__main__':
    ...
    # epoch 0, step 391, train loss 0.573, train acc 0.676, test acc 0.820
    # epoch 1, step 391, train loss 0.397, train acc 0.823: 100%|██████████| 391/391 [00:41<00:00,  9.53it/s]
    # epoch 1, step 391, train loss 0.397, train acc 0.823, test acc 0.836
    # epoch 2, step 391, train loss 0.354, train acc 0.847: 100%|██████████| 391/391 [00:40<00:00,  9.60it/s]
    # epoch 2, step 391, train loss 0.354, train acc 0.847, test acc 0.846
    # epoch 3, step 391, train loss 0.326, train acc 0.861: 100%|██████████| 391/391 [00:40<00:00,  9.64it/s]
    # epoch 3, step 391, train loss 0.326, train acc 0.861, test acc 0.849
    # epoch 4, step 391, train loss 0.322, train acc 0.864: 100%|██████████| 391/391 [00:40<00:00,  9.64it/s]
    # epoch 4, step 391, train loss 0.322, train acc 0.864, test acc 0.848
    # train loss 0.322, train acc 0.864, test acc 0.848
    # 626.3 examples/sec on cuda:0
