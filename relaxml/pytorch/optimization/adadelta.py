...
说明:
https://tech.foxrelax.com/optimization/adadelta/
"""
...
def init_adadelta_states(
    ...
    """
    初始化state, 每个参数需要维护两个状态变量: s_*, delta_*
    """
...
def adadelta(params: List[Tensor], states: Tuple[Tuple[Tensor, Tensor],
                                                 Tuple[Tensor, Tensor]],
    ...
    for p, (s, delta) in zip(params, states):
        with torch.no_grad():
            ...
            s[:] = rho * s + (1 - rho) * torch.square(
                p.grad)  # 更新states.delta_*
            g = (torch.sqrt(delta + eps) / torch.sqrt(s + eps)) * p.grad
            p[:] -= g  # 更新params
            delta[:] = rho * delta + (1 - rho) * g * g  # 更新states.s_*
        p.grad.data.zero_()


def demo1():
    data_iter, feature_dim = load_data_airfoil_self_noise(batch_size=10)
    train_optimization(adadelta, init_adadelta_states(feature_dim),
...
def demo2():
    ...
    train_concise_optimization(trainer, {'rho': 0.9}, data_iter, feature_dim)
