...
说明:
https://tech.foxrelax.com/optimization/rmsprop/
"""
...
def rmsprop_2d(x1: Tensor, x2: Tensor, s1: Tensor,
...
def init_rmsprop_states(feature_dim: int) -> Tuple[Tensor, Tensor]:
    """
    初始化state
    """
...
def rmsprop(params: List[Tensor], states: List[Tensor],
    ...
    for p, s in zip(params, states):
        with torch.no_grad():
            s[:] = gamma * s + (1 - gamma) * torch.square(p.grad)  # 更新states
...
def demo1():
    ...
    demo1()
    """
    show_trace_2d(train_2d(rmsprop_2d), f_2d)
...
def demo2():
    ...
    train_optimization(rmsprop, init_rmsprop_states(feature_dim), {
        'lr': 0.01,
        'gamma': 0.9
