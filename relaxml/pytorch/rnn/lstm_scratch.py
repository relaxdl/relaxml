...
实现说明:
https://tech.foxrelax.com/rnn/lstm_scratch/
"""
...
def get_lstm_params(vocab_size: int, num_hiddens: int,
    ...
    vocab_size: 词表大小
    num_hiddens: 隐藏层大小
    device: 设备(输出的参数会放到这个设备上)

    输出: [W_xi, W_hi, b_i, W_xf, W_hf, b_f, 
          W_xo, W_ho, b_o, W_xc, W_hc, b_c, 
          W_hq, b_q]
    W_xi: [vocab_size, vocab_size]     输入门参数
    W_hi: [num_hiddens, num_hiddens]   输入门参数
    b_i:  [num_hiddens,]               输入门参数
    W_xf: [vocab_size, vocab_size]     遗忘门参数
    W_hf: [num_hiddens, num_hiddens]   遗忘门参数
    b_f:  [num_hiddens,]               遗忘门参数
    W_xo: [vocab_size, vocab_size]     输出门参数
    W_ho: [num_hiddens, num_hiddens]   输出门参数
    b_o:  [num_hiddens,]               输出门参数
    W_xc: [vocab_size, vocab_size]     候选记忆状态参数
    W_hc: [num_hiddens, num_hiddens]   候选记忆状态参数
    b_c:  [num_hiddens,]               候选记忆状态参数
    W_hq: [num_hiddens, vocab_size]    输出层的权重W
    b_q:  [vocab_size, ]               输出层的Bias 
    """
...
def lstm(inputs: Tensor, state: Tuple[Tensor],
         params: List[Tensor]) -> Tuple[Tensor, Tuple[Tensor]]:
    """
    实现LSTM前向传播
    ...
    state:  ([batch_size, num_hiddens],
             [batch_size, num_hiddens]) 状态
    params: 14个参数

    输出: (outputs, state)
    outputs: [num_steps*batch_size, vocab_size] 包含所有时间步的输出
    state: ([batch_size, num_hiddens],
            [batch_size, num_hiddens])          最终的state
    """
...
class RNNModelScratch:
    ...
    >>> device = try_gpu()
    >>> net = RNNModelScratch(vocab_size, num_hiddens, device, get_lstm_params,
    ...
    >>> assert new_state[1].shape == (batch_size, num_hiddens)
    """

    def __init__(self, vocab_size: int, num_hiddens: int, device: torch.device,
                 get_params: Callable, init_state: Callable,
        ...
        num_hiddens: 隐藏层大小
        device: 设备
        get_params: 生成RNN Params的函数
        init_state: 初始化RNN State的函数
        forward_fn: RNN前向传播的函数
        """
    ...
    def __call__(self, X: Tensor,
                 state: Tuple[Tensor]) -> Tuple[Tensor, Tuple[Tensor]]:
        """
        RNN前向传播
...
if __name__ == '__main__':
    run()
# 困惑度 1.2, 31970.9 词元/秒 cuda:0
# time traveller wasnot there i seemed to see a ghostly indistinct
# traveller came to the place reserved for him without aword
