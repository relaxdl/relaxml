...
实现说明:
https://tech.foxrelax.com/rnn/rnn_scratch/
"""
...
def get_rnn_params(vocab_size: int, num_hiddens: int,
    ...
    输出: [W_xh, W_hh, b_h, W_hq, b_q]
    W_xh: [vocab_size, num_hiddens]  输入层的权重W
    W_hh: [num_hiddens, num_hiddens] 隐藏层的权重W
    b_h:  [num_hiddens, ]            隐藏层的Bias
...
def rnn(inputs: Tensor, state: Tuple[Tensor],
...
class RNNModelScratch:
    ...
    >>> device = try_gpu()
    >>> net = RNNModelScratch(vocab_size, num_hiddens, device, get_rnn_params,
                              init_rnn_state, rnn)
...
def run() -> None:
    ...
    net = RNNModelScratch(len(vocab), num_hiddens, device, get_rnn_params,
                          init_rnn_state, rnn)
    kwargs = {
        'num_epochs': 500,
        'lr': 1,
...
if __name__ == '__main__':
    run()
# 困惑度 2.3, 112470.2 词元/秒 cuda:0
# time traveller of the whole dozen pattent fat even was the moon
# traveller asked and it milation the floor did not look coul
