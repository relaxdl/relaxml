...
实现说明:
https://tech.foxrelax.com/rnn/gru_scratch/
"""
...
def get_gru_params(vocab_size: int, num_hiddens: int,
    ...
    输出: [W_xz, W_hz, b_z, W_xr, W_hr, b_r, W_xh, W_hh, b_h, W_hq, b_q]
    W_xz: [vocab_size, vocab_size]   更新门参数
    W_hz: [num_hiddens, num_hiddens] 更新门参数
    b_z:  [num_hiddens, ]            更新门参数
    W_xz: [vocab_size, vocab_size]   重置门参数
    W_hz: [num_hiddens, num_hiddens] 重置门参数
    b_z:  [num_hiddens, ]            重置门参数
    W_xz: [vocab_size, vocab_size]   候选隐藏状态参数
    W_hz: [num_hiddens, num_hiddens] 候选隐藏状态参数
    b_z:  [num_hiddens, ]            候选隐藏状态参数
...
class RNNModelScratch:
    ...
    >>> device = try_gpu()
    >>> net = RNNModelScratch(vocab_size, num_hiddens, device, get_gru_params,
                              init_gru_state, gru)
...
def run() -> None:
    ...
    net = RNNModelScratch(len(vocab), num_hiddens, device, get_gru_params,
                          init_gru_state, gru)
    kwargs = {
        'num_epochs': 500,
        'lr': 1,
...
if __name__ == '__main__':
    run()
# 困惑度 1.1, 37805.2 词元/秒 cuda:0
# time traveller i m funny be all right in a minutehe put down his
# traveller s shoulderyou don t believe itwelli thought notth